...
public class BathroomServiceImpl implements BathroomService {
	...
	@Value("classpath:config/Bathroom/allBathroom.sql")
	private Resource allBathroomSql;
	
	@Value("classpath:config/Bathroom/eachBathroom.sql")
	private Resource eachBathroomSql;
	...
	public Map<String,Object> getPieData(boolean flag){
		...
		try{
			String re = getPieResult("各浴室当前人数分布情况",allBathroomSql);
			reMap.put("allBathroom", re);
		}catch(Exception e){
			e.printStackTrace();
		}
		
		try{
			String re = getEachPieResult("浴室一当前人数分布情况",eachBathroomSql);
			reMap.put("eachBathroom1", re);
		}catch(Exception e){
			e.printStackTrace();
		}
		
		try{
			String re = getEachPieResult("浴室二当前人数分布情况",eachBathroomSql);
			reMap.put("eachBathroom2", re);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	private String getEachPieResult(String caption,Resource Rsql) {
		...
		if(caption.equals("浴室一当前人数分布情况")){
			id = "1";
			max = "80";
			
		}
		else if(caption.equals("浴室二当前人数分布情况")){
			id = "2";
			max = "60";
		}
