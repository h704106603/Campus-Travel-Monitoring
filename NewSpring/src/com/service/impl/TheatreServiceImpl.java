...
public class TheatreServiceImpl implements TheatreService{
    ...
    @Value("classpath:config/Theatre/alltheatre.sql")
    private Resource allTheatreSql;
    
    @Value("classpath:config/Theatre/eachTheatre.sql")
    private Resource eachTheatreSql;
    
    @Value("classpath:config/Theatre/theatreBar.sql")
    private Resource theatreBarSql;
    ...
    public Map<String,Object> getData(boolean flag){
        ...
        try{
            String re = getBar("130近两小时人数明细",theatreBarSql,"1");
            reMap.put("130Theatre", re);
        }catch(Exception e){
            e.printStackTrace();
        }
        
        try{
            String re = getBar("146近两小时人数明细",theatreBarSql,"2");
            reMap.put("146Theatre", re);
        }catch(Exception e){
            e.printStackTrace();
        }
    ...
    public Map<String,Object> getPieData(boolean flag){
        ...
        try{
            String re = getPieResult("各剧场当前人数分布情况",allTheatreSql);
            reMap.put("allTheatre", re);
        }catch(Exception e){
            e.printStackTrace();
        }
        
        try{
            String re = getEachPieResult("130剧场当前人数分布情况",eachTheatreSql);
            reMap.put("eachTheatre130", re);
        }catch(Exception e){
            e.printStackTrace();
        }
        
        try{
            String re = getEachPieResult("146剧场当前人数分布情况",eachTheatreSql);
            reMap.put("eachTheatre146", re);
        }catch(Exception e){
            e.printStackTrace();
        }
        
        try{
            String re = getEachPieResult("346剧场当前人数分布情况",eachTheatreSql);
            reMap.put("eachTheatre346", re);
        }catch(Exception e){
            e.printStackTrace();
        }
    ...
    private String getEachPieResult(String caption,Resource Rsql) {
        ...
        if(caption.equals("130剧场当前人数分布情况")){
            id = "1";
            max = "300";
            
        }
        else if(caption.equals("146剧场当前人数分布情况")){
            id = "2";
            max = "400";
        }
        else if(caption.equals("346剧场当前人数分布情况")){
            id = "3";
            max = "80";
        }
